...
function searchProducts(query) {
    ...
    return originalProducts.filter(function (product) {
        return product.ten_san_pham.toLowerCase().includes(searchTerm) ||
...
function handleSearch2() {
    ...
    var filteredProducts = searchProducts(query);
    displaySearchResults(filteredProducts);
    // Thêm thông tin số lượng kết quả tìm kiếm (tùy chọn)
    var resultInfo = document.getElementById('searchResultInfo');
    if (resultInfo) {
        if (query.trim() === '') {
            resultInfo.textContent = "Hi\u1EC3n th\u1ECB t\u1EA5t c\u1EA3 ".concat(originalProducts.length, " s\u1EA3n ph\u1EA9m");
        }
        else {
            resultInfo.textContent = "T\u00ECm th\u1EA5y ".concat(filteredProducts.length, " s\u1EA3n ph\u1EA9m cho \"").concat(query, "\"");
        }
    }
}
...
window.handleSearch2 = handleSearch2;
